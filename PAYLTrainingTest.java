...
public class PAYLTrainingTest
{
	...
	public static void main (String args[]) throws Exception
	{
		...
		job.setJarByClass(PAYLTrainingTest.class);
		job.setJobName("Training of PAYL");
		...
		PCAPFileInputFormat.addInputPath(job, new Path(inputPath1));
		//PCAPFileInputFormat.addInputPath(job, new Path(inputPath2));

		job.setOutputFormatClass(PAYLFileOutputFormat.class);
		PAYLFileOutputFormat.setOutputPath(job, new Path(outputPath));
		job.setMapperClass(PAYLTraining.PAYLMapper.class);
		job.setReducerClass(PAYLTraining.PAYLReducer.class);
