 ...
 * File Name: PAYLAvgFTraining.java
 * Course: COMP790-042
...
public class PAYLAvgFTraining
{
    ...
    public static class PAYLReducer extends Reducer <PAYLKey, PAYLVWritable, NullWritable, PAYLAvgFWritable>
    {
	private MultipleOutputs <NullWritable, 
				 PAYLAvgFWritable> multipleOutputs;
	private double[] freq;
	private double avgByteFreq[];
	private int i = 0;
	private int FREQ_ARRAY_SIZE = 256;
	private PAYLAvgFWritable wr;

	protected void setup(Context context) throws IOException,
						InterruptedException
	{
	    freq = new double[FREQ_ARRAY_SIZE];
	    multipleOutputs = new MultipleOutputs <NullWritable, 
						PAYLAvgFWritable> (context);
	    avgByteFreq = new double [FREQ_ARRAY_SIZE];
	    wr = new PAYLAvgFWritable(0);
	}
			    ...
			    throws IOException, InterruptedException
	{
		...
		for (i = 0; i < FREQ_ARRAY_SIZE; i++) {
		    freq[i] += ((double)value.freq[i]/value.payloadSize);	
		}
		noOfPackets++;
	    }

            for (i = 0; i < FREQ_ARRAY_SIZE; i++) {
                avgByteFreq[i] = freq[i]/noOfPackets;
            }
